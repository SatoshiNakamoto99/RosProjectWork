...
class AudioNode(object):
    ...
    def _handle_pepper_talk(self, pepper_talk):
        ...
        if self._get_start_listen() and self._get_pepper_talk():
            self._set_pepper_talk_during_listening(True)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self._pepper_talk:
                continue
            self._set_start_listen(True)              
            text = self._start_listening()
            if self._get_pepper_talk_during_listening():
                self._set_pepper_talk_during_listening(False)
                self._set_start_listen(False)
                continue
